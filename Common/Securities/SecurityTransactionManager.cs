...
namespace QuantConnect.Securities
{
    ...
    public class SecurityTransactionManager : IOrderProvider
    {
        ...
        public bool WaitForOrder(int orderId)
        {
            ...
            //todo: Support LTC. There must be a way to determine market
            if (orderTicket.Symbol.Value.StartsWith("BTC") || orderTicket.Symbol.Value.EndsWith("BTC"))
            {
                if (!orderTicket.OrderFilled.WaitOne(10000))
                {
                    Log.Error("SecurityTransactionManager.WaitForOrder(): Order did not fill within 10 seconds.");
                    return false;
                }
            }
            else
