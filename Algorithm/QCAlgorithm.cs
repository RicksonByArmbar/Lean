...
namespace QuantConnect.Algorithm
{
    ...
    public partial class QCAlgorithm : MarshalByRefObject, IAlgorithm
    {
        ...
        public void SetBrokerageModel(BrokerageName brokerage, AccountType accountType = AccountType.Margin)
        {
            switch (brokerage)
            {
                ...
                case BrokerageName.Bitfinex:
                    BrokerageModel = new BitfinexBrokerageModel();
                    break;
        ...
        public void AddBitcoinData<T>(string symbol, Resolution resolution, DateTimeZone timeZone, bool fillDataForward = false, decimal leverage = 1.0m)
           where T : BaseData, new()
        {
            if (_locked) return;

            //Add this to the data-feed subscriptions
            var symbolObject = new Symbol(SecurityIdentifier.GenerateForex(symbol, Market.Bitcoin), symbol);

            //Add this new generic data as a tradeable security: 
            var security = SecurityManager.CreateSecurity(typeof(T), Portfolio, SubscriptionManager, SecurityExchangeHours.AlwaysOpen(timeZone), timeZone, symbolObject, resolution,
                fillDataForward, leverage, true, false, true);
